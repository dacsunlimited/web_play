...
class BlockchainAPI
  ...
  # Get the account fee for a account name
  # parameters:
  #   string `account_name` - the account name to register
  # return_type: `asset`
  fee_register_account: (account_name, error_handler = null) ->
    @rpc.request('blockchain_fee_register_account', [account_name]).then (response) ->
      response.result
